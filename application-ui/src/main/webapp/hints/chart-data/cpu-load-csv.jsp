...
%>"Model","CPU Avg","Count"<sql:query var="hits" dataSource="jdbc/impala">
SELECT devicemodel, AVG(cpu_load) AS avgcpuload, modelcount
FROM devicestatus
INNER JOIN (
...
"<c:out value="${row.devicemodel}"/>",<c:out value="${row.avgcpuload}"/>,<c:out value="${row.modelcount}"/>
</c:forEach>
